  ...
  <div>
    <div v-if="task.app_name === 'umamusume'">
      <div>
        ...
        <div v-if="task.task_execute_mode === 'CRON_JOB'" class="small time">
          下次执行时间：{{ task.cron_job_info?.next_time }} ({{ task.cron_job_info?.cron }})
        </div>
      </div>
      ...
      <div
          v-if="task.detail.cultivate_result!==undefined &&task.detail.cultivate_result.factor_list !== undefined && task.detail.cultivate_result.factor_list.length !== 0">
        因子获取：<span class="mr-1" v-for="factor in task.detail.cultivate_result.factor_list">
          <span
              v-if="factor[0] === '速度' || factor[0] === '耐力'|| factor[0] === '力量'|| factor[0] === '毅力'|| factor[0] === '智力'"
              style="background-color: #49BFF7;"
              class="badge badge-pill badge-secondary">{{ factor[0] }}({{ factor[1] }})</span>
          <span
              v-if="factor[0] === '短距离' || factor[0] === '英里'|| factor[0] === '中距离'|| factor[0] === '长距离'|| factor[0] === '泥地'|| factor[0] === '草地'|| factor[0] === '领跑'|| factor[0] === '跟前'|| factor[0] === '居中'|| factor[0] === '后追'"
              style="background-color: #FF78B2;"
              class="badge badge-pill badge-secondary">{{ factor[0] }}({{ factor[1] }})</span>
          <span
              v-if="factor[0] !== '速度' && factor[0] !== '耐力'&& factor[0] !== '力量'&& factor[0] !== '毅力'&& factor[0] !== '智力'&& factor[0] !== '短距离' && factor[0] !== '英里'&& factor[0] !== '中距离'&& factor[0] !== '长距离'&& factor[0] !== '泥地'&& factor[0] !== '草地' &&factor[0] !== '领跑'&& factor[0] !== '跟前'&& factor[0] !== '居中'&& factor[0] !== '后追'"
              style="background-color: #E0E0E0; color: #794016;"
              class="badge badge-pill badge-secondary">{{ factor[0] }}({{ factor[1] }})</span>
        </span>
      </div>
    </div>
